...
inline constexpr float JUMPIMPULSE{-7.75f * TILESIZE};
inline constexpr float FRICTION{50.0f * TILESIZE}; // velocity/sec removed when no input (moveToward style)
inline constexpr float MAXSPEED{TILESIZE * 4.0f};
inline constexpr float AIR_CONTROL{0.12f};        // fraction of ground accel available in air
